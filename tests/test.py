...
def test_handle_delete_task_question_mocked():
    assistant = PersonalAssistant.load_state(name="test2", confirm_callback=lambda msg: "כן")
    assistant._todo_list = [{"description": "פגישה חשובה", "time": "10/10/2025 09:00"}]
    fake_gpt_output = {
        "index": 0,
        "description": "פגישה חשובה"
    }
...
def test_handle_delete_task():
    pa = PersonalAssistant.load_state(TEST_NAME, confirm_callback=lambda msg: "כן")
    pa._todo_list = [{"description": "פגישה חשובה", "time": "10/10/2025 09:00"}]
    pa._todo_list = [
...
def test_handle_clear_tasks_yes():
    pa = PersonalAssistant.load_state(TEST_NAME, confirm_callback=lambda msg: "כן")
    pa._todo_list = [{"description": "משימה", "time": None}]
...
def test_keep_chat_history():
    ...
    assert pa._messages[-2]["content"] == "מה השעה?"
    assert pa._messages[-1]["content"] == "עכשיו 12:00"


# === SAVE / LOAD ===
def test_save_and_load_state():
    pa = PersonalAssistant.load_state(TEST_NAME, confirm_callback=lambda msg: "כן")
    pa._todo_list = [{"description": "בדיקה", "time": None}]
    pa._messages.append({"role": "user", "content": "שלום"})
    pa.save_state()

    pa2 = PersonalAssistant.load_state(TEST_NAME, confirm_callback=lambda msg: "כן")
    assert pa2._todo_list == pa._todo_list
    assert pa2._messages[-1]["content"] == "שלום"
...
def test_handle_reset_question_yes():
    ...
    assert len(pa._todo_list) == 0
    assert pa._messages[-1]["role"] == "system"
...
def test_load_existing_agent_state():
    ...
    assert pa._todo_list == initial_tasks
